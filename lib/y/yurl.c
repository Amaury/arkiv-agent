 ...
 * Parse a given URL.
 */
yurl_t *yurl_parse(const char *url, bool strict) {
	yurl_t *res;
	const char *pt, *pt2;
	char *arobase, *colon;
	int len;
	...
	/* search for protocol */
	if ((pt = strstr(url, "://"))) {
		for (int i = 0; _yurl_proto_table[i].string; ++i) {
	...
	for (int i = 0; _yurl_proto_table[i].string; ++i) {
		if (proto == _yurl_proto_table[i].proto)
			return (_yurl_proto_table[i].string ?
			        strdup(_yurl_proto_table[i].string) : NULL);
	}
	return (NULL);
}
/*
...
char *yurl_assemble(yurl_t *url) {
	...
	free0(proto);
	if (ys_append(&str1, "://") != YENOERR) {
		ys_free(str1);
		return (NULL);
	}
	if (url->login &&
	    (ys_append(&str1, url->login) != YENOERR ||
	     (url->auth &&
	      (ys_append(&str1, ";auth=") != YENOERR ||
	       ys_append(&str1, url->auth) != YENOERR)) ||
	     (url->pass &&
	      (ys_append(&str1, ":") != YENOERR ||
	       ys_append(&str1, url->pass) != YENOERR)) ||
	     ys_append(&str1, "@") != YENOERR)) {
		ys_free(str1);
		return (NULL);
	}
	if (url->host && ys_append(&str1, url->host) != YENOERR) {
		ys_free(str1);
		return (NULL);
	}
	if (url->port != port) {
		...
		if (!ys_printf(&str2, "%s%d", ":", url->port) || ys_append(&str1, (char*)str2) != YENOERR) {
			ys_free(str2);
			ys_free(str1);
			return (NULL);
		}
	...
	if ((url->location && ys_append(&str1, url->location) != YENOERR) ||
	    (!url->location && ys_append(&str1, "/") != YENOERR)) {
		ys_free(str1);
		return (NULL);
	}
	if (url->query && (ys_append(&str1, "?") != YENOERR || ys_append(&str1, url->query) != YENOERR)) {
		ys_free(str1);
		return (NULL);
	}
	res = ys_string(str1);
...
ystr_t yurl_encode(const char *url) {
	...
	for (pt = url; *pt; ++pt) {
		if (*pt == SPACE) {
			ys_addc(&res, PLUS);
		} else if (isalnum(*pt) || *pt == DOT || *pt == MINUS || *pt == UNDERSCORE) {
			ys_addc(&res, *pt);
		} else {
