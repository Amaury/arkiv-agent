...
/* Private prototypes */
static void _ydom_open_hdlr(ysax_t *sax, char *tag_name, yarray_t attrs);
 ...
 */
ystatus_t ydom_read_file(ydom_t *dom, const char *filename) {
	ysax_t *sax;
 ...
 */
ystatus_t ydom_read_stream(ydom_t *dom, FILE *stream) {
	ysax_t *sax;
 ...
 */
ystatus_t ydom_read_memory(ydom_t *dom, char *mem) {
	ysax_t *sax;
...
ystr_t ydom_dump(ydom_t *dom) {
	...
	if (dom->xml_version) {
		ys_append(&str, " version=\"");
		ys_append(&str, dom->xml_version);
		ys_addc(&str, '\"');
	}
	if (dom->encoding) {
		ys_append(&str, " encoding=\"");
		ys_append(&str, dom->encoding);
		ys_addc(&str, '\"');
	}
	if (dom->standalone) {
		ys_append(&str, " standalone=\"");
		ys_append(&str, dom->standalone);
		ys_addc(&str, '\"');
	}
	ys_append(&str, "?>");
 ...
 */
yarray_t ydom_xpath(ydom_t *dom, char *xpath) {
	ydom_node_t *node;
 ...
 * existing node.
 */
ydom_node_t *ydom_node_add_process_instr(ydom_node_t *node, char *target, char *data) {
	ydom_node_t *new_node;
 ...
 * as root.
 */
yarray_t ydom_node_xpath(ydom_node_t *node, char *xpath) {
	yarray_t res;

	YLOG_ADD(YLOG_DEBUG, "Entering");
	if (!node || !(res = yarray_new()))
		return (NULL);
	yarray_push(&res, node);
	res = ydom_get_nodes_from_xpath(res, xpath);
	/*
	if (!yarray_length(res)) {
		yarray_free(&res, NULL, NULL);
		res = NULL;
	}
	*/
	YLOG_ADD(YLOG_DEBUG, "Exiting");
...
void ydom_node_sort(ydom_node_t *node, int (*func)(const void *, const void*)) {
	...
	yarray_t array;
	char str_star[2] = {ASTERISK, '\0'};

	if ((array = ydom_node_xpath(node, str_star))) {
		if (array && yarray_length(array)) {
			yarray_sort(array, func);
			for (prev = NULL; (pt = yarray_pop(array)); ) {
				next = array[0];
				pt->prev = prev;
				pt->next = next;
				if (!prev)
...
static void _ydom_process_instr_hdlr(ysax_t *sax, char *target, char *content) {
	...
	if (!strcmp(target, XML)) {
		if (content && (pt = strstr(content, XML_VERSION))) {
			if ((pt = strchr(pt, EQ)) &&
			    ((pt2 = strchr(pt, DQUOTE)) || (pt2 = strchr(pt, QUOTE))) &&
			    ((pt3 = strchr(pt2 + 1, DQUOTE)) || (pt3 = strchr(pt2 + 1, QUOTE)))
			   ) {
				if ((dom->xml_version = malloc0(pt3 - pt2))) {
					ptrdiff_t len = pt3 - pt2 - 1;
					memcpy(dom->xml_version, pt2 + 1, len);
					dom->xml_version[len] = '\0';
				}
			}
		}
		if (content && (pt = strstr(content, ENCODING))) {
			if ((pt = strchr(pt, EQ)) &&
				...
				if ((dom->encoding = malloc0(pt3 - pt2))) {
					ptrdiff_t len = pt3 - pt2 - 1;
					memcpy(dom->encoding, pt2 + 1, len);
					dom->encoding[len] = '\0';
				}
			}
		}
		if (content && (pt = strstr(content, STANDALONE))) {
			if ((pt = strchr(pt, EQ)) &&
				...
				if ((dom->standalone = malloc0(pt3 - pt2))) {
					ptrdiff_t len = pt3 - pt2 - 1;
					memcpy(dom->standalone, pt2 + 1, len);
					dom->standalone[len] = '\0';
				}
			}
		}
	} else {
