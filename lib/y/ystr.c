...
/* Create an empty ystring, defining the size of its buffer. */
ystr_t ys_create(size_t size) {
	char *res;
	ystr_head_t *y;

	if (size < YSTR_MINIMAL_SIZE)
		size = YSTR_MINIMAL_SIZE;
	else
		size = (((size / YSTR_MINIMAL_SIZE) + 1) * YSTR_MINIMAL_SIZE) + 1;
	res = (char*)malloc0(size + sizeof(ystr_head_t));
	if (!res)
		return (NULL);
	y = (ystr_head_t*)res;
	res += sizeof(ystr_head_t);
...
ystr_t ys_urlencode(const char *str) {
	if (!str)
		return (NULL);
	size_t inputLen = strlen(str);
	ystr_t res = ys_create(inputLen);
	if (!res)
		return (NULL);
...
ystatus_t ys_gets(ystr_t *s, FILE *stream) {
	...
	while (true) {
		if (!fgets(buffer, 4096, stream))
			return (YENOERR);
		bool endOfLine = (buffer[strlen(buffer) - 1] == '\n') ? true : false;
		ys_append(s, buffer);
		ys_rtrim(*s);
		if (endOfLine)
			return (YENOERR);
